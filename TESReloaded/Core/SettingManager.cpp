...
#define kWriteSetting 0x00C43E90
...
#define kWriteSetting 0x004A85F0
...
SettingManager::SettingManager() {
	...
	SettingsMain.CameraMode.OneHandAimingOffset.y = atof(value);
	GetPrivateProfileStringA("CameraMode", "OneHandAimingOffsetZ", "5.0", value, SettingStringBuffer, Filename);
	SettingsMain.CameraMode.OneHandAimingOffset.z = atof(value);
	GetPrivateProfileStringA("CameraMode", "TwoHandAimingOffsetX", "0.0", value, SettingStringBuffer, Filename);
	SettingsMain.CameraMode.TwoHandAimingOffset.x = atof(value);
	GetPrivateProfileStringA("CameraMode", "TwoHandAimingOffsetY", "15.0", value, SettingStringBuffer, Filename);
...
void SettingManager::LoadSettings() {
	...
	GetPrivateProfileStringA("Interiors", "MinRadius", "10.0", value, SettingStringBuffer, Filename);
	SettingsShadows.Interiors.Forms.MinRadius = atof(value);
	SettingsShadows.Interiors.Quality = GetPrivateProfileIntA("Interiors", "Quality", 0, Filename);
	SettingsShadows.Interiors.LightPoints = GetPrivateProfileIntA("Interiors", "LightPoints", 2, Filename);
			...
			if (!FormStatus) {
				...
				if (FormType[0] == 'X' || FormType[0] == 'E') {
					if (i == 0)
						...
						SettingsShadows.Exteriors.ExcludedForms.push_back(strtol(FormID, NULL, 16));
				}
				if (FormType[0] == 'X' || FormType[0] == 'I') {
					if (i == 0)
						IC += 1;
					else
						SettingsShadows.Interiors.ExcludedForms.push_back(strtol(FormID, NULL, 16));
				}
			}
			pNextSection = pNextSection + strlen(pNextSection) + 1;
		}
...
#elif defined (OBLIVION)
			WritePrivateProfileStringA("CameraMode", "BowAimingOffsetX", ToString(SettingsMain.CameraMode.BowAimingOffset.x).c_str(), SettingsMain.Main.MainFile);
			WritePrivateProfileStringA("CameraMode", "BowAimingOffsetY", ToString(SettingsMain.CameraMode.BowAimingOffset.y).c_str(), SettingsMain.Main.MainFile);
			WritePrivateProfileStringA("CameraMode", "BowAimingOffsetZ", ToString(SettingsMain.CameraMode.BowAimingOffset.z).c_str(), SettingsMain.Main.MainFile);
#endif
#if defined(SKYRIM)
	...
	if (!strcmp(Item, "Main")) {
		if (!strcmp(Definition, "Main")) {
			if (!strcmp(Section, "Main")) {
				#if defined(OBLIVION)
				Settings["FarPlaneDistance"] = SettingsMain.Main.FarPlaneDistance;
				#endif
...
#elif defined(OBLIVION)
				Settings["BowAimingOffsetX"] = SettingsMain.CameraMode.BowAimingOffset.x;
				Settings["BowAimingOffsetY"] = SettingsMain.CameraMode.BowAimingOffset.y;
				Settings["BowAimingOffsetZ"] = SettingsMain.CameraMode.BowAimingOffset.z;
#endif
			}
...
void SettingManager::SetMenuSetting(const char* Item, const char* Definition, const char* Section, UInt32 SelectedSectionKey, const char* Setting, float Value) {
	...
	if (!strcmp(Item, "Main")) {
		if (!strcmp(Definition, "Main")) {
			if (!strcmp(Section, "Main")) {
				if (!strcmp(Setting, "ScreenshotKey"))
				...
				else if (!strcmp(Setting, "DialogOffsetX"))
					SettingsMain.CameraMode.DialogOffset.x = Value;
				else if (!strcmp(Setting, "DialogOffsetY"))
				...
				else if (!strcmp(Setting, "BowAimingOffsetX"))
					SettingsMain.CameraMode.BowAimingOffset.x = Value;
				else if (!strcmp(Setting, "BowAimingOffsetY"))
					SettingsMain.CameraMode.BowAimingOffset.y = Value;
				else if (!strcmp(Setting, "BowAimingOffsetZ"))
					SettingsMain.CameraMode.BowAimingOffset.z = Value;
			}
			else if (!strcmp(Section, "EquipmentMode")) {
...
class Settings {
public:
	bool TrackReadSetting(GameSetting*);
	bool TrackWriteSetting(GameSetting*);
#if defined(OBLIVION)
	bool TrackLoadGame(BSFile*, char*, UInt8);
#elif defined(SKYRIM) || defined(NEWVEGAS)
	bool TrackLoadGame(char*, UInt8);
#endif
};
...
bool (__thiscall Settings::* WriteSetting)(GameSetting*);
bool (__thiscall Settings::* TrackWriteSetting)(GameSetting*);
bool Settings::TrackWriteSetting(GameSetting* Setting) {
...
void CreateSettingsHook() {
	...
	*((int *)&WriteSetting)		= kWriteSetting;
	TrackWriteSetting			= &Settings::TrackWriteSetting;
